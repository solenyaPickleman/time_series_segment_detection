...
from tqdm import tqdm
#read in data
dat = pd.read_csv('C:/Users/brady/Downloads/ExportCSV.csv')
dat.columns= ['date','value']
dat = dat.sort_values(by=['date'])
#dat = dat
...
test = training[280:]
test_targets = targets[280:]
training = training[:280]
training_targets = targets[:280]
...
test_targets = torch.Tensor(targets).to(device)  # a dummy target, for example

#test accuracy on test set
accuracy =0 
for i,data in tqdm(enumerate(training)):
        inputs = torch.Tensor(data)
        h,w = inputs.shape
        inputs = inputs.reshape(1,1,h,w)
        labels = test_targets[i].view(1,-1)
...
print("Model accuracy on test data: " ,accuracy/len(training))
